...
import { getCategory, getMarbleDesigns, type CategoryRecord } from "@/lib/designsData";

export default function MarbleDesignsPage() {
  const [designs, setDesigns] = useState<MarbleDesign[]>([]);
  const [category, setCategory] = useState<CategoryRecord | null>(null);
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    Promise.all([getMarbleDesigns(), getCategory("marble")])
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="relative px-6 py-10 md:px-10 md:py-14 flex items-center justify-between gap-6">
            <div>
              ...
              <h1 className="text-3xl md:text-4xl font-semibold text-[#1a1712] mb-2">
                {category?.name ?? "Marble"} Pool Tables
              </h1>
              <p className="text-[#5a5044] max-w-2xl">
                {category?.description ||
                  "Start by choosing a marble table design, then fine-tune details like cloth color and wood accents on the next step."}
              </p>
            </div>
            <Link
              href="/"
        ...
        {loading && designs.length === 0 ? (
          ...
          <div className="rounded-3xl border border-[#e7dccb] bg-white px-8 py-10 text-center">
            <p className="text-[#5a5044]">No marble designs yet.</p>
